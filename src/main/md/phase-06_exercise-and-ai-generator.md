...
- SubmitAnswerResult submitAnswer(UUID userId, UUID attemptId, UUID questionId, String answer)
...
Fields: topic (String), exerciseType (ExerciseType), difficulty (Level), questionCount (int 1-20), relatedLessonId (UUID, optional)
...
- POST /exercises/attempt/{attemptId}/answer — HTMX: submit single answer, return feedback partial
- POST /exercises/attempt/{attemptId}/complete — complete, redirect to result page
...
exercises/fragments/question-card.html — renders per ExerciseType
exercises/fragments/answer-feedback.html — HTMX target after answer submission
...
2. Controller calls ExerciseService.generateAiExercise() (async)
3. HTMX shows loading spinner via hx-trigger="load"
4. When complete, redirect to new exercise detail page
5. If AI fails, show error partial, offer retry
